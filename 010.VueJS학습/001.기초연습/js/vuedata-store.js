...
/* 
    ...
    아래와 같은 MIME 전송형식 type 에러가 발생함!
    ______________________________________________
    iled to load module script: Expected a JavaScript module script but the server responded with a MIME type of "application/json". Strict MIME type checking is enforced for module scripts per HTML spec.
...
const store = new Vuex.Store({
    ...
    mutations:{
        ...
        setData(st,pm){ // st - state 변수, pm - 전달변수
            // state 구역의 items 변수에 제이슨 데이터 담기!
            st.items = pm;
            console.log("뮤테:",pm);
