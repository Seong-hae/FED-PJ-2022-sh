...
/**********************************************************************
    ...
    1. 데이터와 데이터상태를 한 번에 관리하는 확장 라이브러리!
    2. 스토어에서 관리되는 데이터는 리액티브 데이터이다!
    ( reactive data 양방향 동기화 데이터이다! 상호작용! )
    ...
    3. actions (활동) : 비동기처리 메서드 구역

    [ 뷰엑스 스토어 처리순서 ]
    1. actions -> 2. mutations -> 3. state
    - 흐름 : 먼저 비동기 메서드처리 후
            일반 메서드 처리 결과를 state 변수에 반영함!
    (참고: 뷰엑스 관리흐름도
    ...
    예시코드)
        ...
        [ 뷰엑스 스토어 버전이슈! ]
        - 일반적으로 뷰JS 버전이 2.x 이면 vueX는 3.x 버전을 씀
        (예컨데 최신 뷰는 3.x이므로 vuex는 4.x버전을 씀)
        -> 메이저버전 숫자(첫번째 숫자)가 뷰보다 하나 크다!
        (주의 : webpack 템플릿을 사용한 SPA 구축환경에서
            이 버전이 맞지않으면 뷰엑스 스토어에서 에러가 발생함!)

        - 재밌는 것이 있음......!
        내보낼 때 const store = 뷰엑스 스토어
