...
/***************************************************************** 
    ...
    - 변경사항들
    1. 공용 변수는 constructor 에 this 키워드로 등록함
    2. 모든 함수는 메서드형태로 변경함
        function 이름(){}  ->  이름(){}
    3. 서로다른 메서드에서 클래스내부 다른 메서드를 호출할 때
        this 키워드를 사용하여 호출함!
    4. 이벤트 등록시 addEventListener(이벤트명,함수)
        함수일 때는 함수명만 쓰면 등록되었으나 클래스는 함수가 아닌 메서드이므로
        이것은 익명함수 구역을 만들고 함수를 호출하는 방식으로 this 키워드를
        사용한 메서드 호출을 해야 호출됨!
    

*****************************************************************/
class AutoScroll {
    // 생성자 메서드 (최초실행구역이 되기도 함!)
    constructor() {
