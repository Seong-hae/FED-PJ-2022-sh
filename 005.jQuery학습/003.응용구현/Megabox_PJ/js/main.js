...
$(function () { //// jQB2 //////////////////////////
    ...
    $(".btnpp img").click(function(){
        ...
        if(!paused_sts){ // false일떄 들어오려면 NOT(!)연산자 사용

            // 비디오 멈추기는 pause() 메서드!
            mv.get(0).pause();
            // 멈춤상태일 경우 진한 재생버튼으로 변경!
    ...
    mv.on("timeupdate",function(){
        // 목표 : 비디오가 재생중일때 진행바가 움직이게 한다
        // 원리 : 진행바의 진행비율을 %로 나타내야 한다!
        // 계산법 : 현재시간 / 전체시간 * 100 -> 백분율(%)

        // 1. 비디오 현재시간 가져오기 : currentTime 속성(비디오 현재시간)
        let ctime = $(this)[0].currentTime;
        // mv[0] === mv.get(0)
        // 비교) document.querySeletorAll(".my").item(0)
        // === document.querySeletorAll(".my")[0]
